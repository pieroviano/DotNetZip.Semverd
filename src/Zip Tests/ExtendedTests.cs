...
namespace Ionic.Zip.Tests.Extended
{
    ...
    {
        public ExtendedTests(Xunit.Abstractions.ITestOutputHelper output) : base(output) { }
        ...
        public void Bzip2_Perf()
        {
            ...
            for (int k = 0; k < 2; k++)
            {
                ...
                var msg = $"test BZip2 perf check, cycle {k + 1}/2 (est. time: 22 mins)";
                _txrx.Send(msg);
        ...
        public void TestZip_IsZipFile()
        {
            ...
            {
                filename = Path.Combine(subdir, $"FileToBeAdded-{j:D2}.txt");
            ...
            filename = Path.Combine(subdir, $"ThisFileDoesNotExist.{_rnd.Next(2000):D2}.txt");
            Assert.IsTrue(!ZipFile.IsZipFile(filename),
        ...
        public void Create_RenameRemoveAndRenameAgain_wi8047()
        {
            ...
            for (int m = 0; m < 2; m++)
            {
                string zipFileToCreate = Path.Combine(TopLevelDir, $"Create_RenameRemoveAndRenameAgain_wi8047-{m}.zip");
        ...
        public void Create_EmitTimestampOptions()
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                for (int k = 0; k < 3; k++)
                {
                    string zipFileToCreate = Path.Combine(TopLevelDir, $"Create_EmitTimestampOptions-{j}-{k}.zip");
                    using (var zip = new ZipFile())
                    {
        ...
        public void Test_AddUpdateFileFromStream()
        {
            ...
            {
                string zipFileToCreate = Path.Combine(TopLevelDir, $"Test_AddUpdateFileFromStream-{k}.zip");
                ...
                using (ZipFile zip1 = new ZipFile())
                {
                    ...
                    {
                        zip1.AddEntry($"Lorem{i + 1}.txt", inputStrings[i]);
                    }
                    zip1.Save(zipFileToCreate);
                }
        ...
        public void Test_AddEntry_String()
        {
            ...
            for (int a = 0; a < crypto.Length; a++)
            {
                for (int b = 0; b < passwords.Length; b++)
                {
                    ...
                    {
                        string zipFileToCreate = Path.Combine(TopLevelDir, $"Test_AddEntry_String-{a}.{b}.{c}.zip");
        ...
        public void Test_AddDirectoryByName()
        {
            for (int n = 1; n <= 10; n++)
            {
                ...
                string zipFileToCreate = Path.Combine(TopLevelDir, $"Test_AddDirectoryByName{n:N2}.zip");
                using (ZipFile zip1 = new ZipFile())
                {
                    for (int i = 0; i < n; i++)
                    {
        ...
        public void Create_WithEvents()
        {
            ...
            int subdirCount = 0;
            int entriesAdded = TestUtilities.GenerateFilesOneLevelDeep("Create_WithEvents", dirToZip, randomizerSettings, null, out subdirCount);
            ...
            for (int m = 0; m < 2; m++)
            {
                ...
                string zipFileToCreate = Path.Combine(TopLevelDir, $"Create_WithEvents-{m}.zip");
        ...
        public void CreateZip_AddDirectory_NoFilesInRoot_WI5893()
        {
            ...
            for (i = 0; i < subdirCount; i++)
            {
                ...
                {
                    String file = Path.Combine(subdir, $"file{j:D3}.a");
        ...
        public void Create_AddDirectory_NoFilesInRoot_WI5893a()
        {
            ...
            for (i = 0; i < subdirCount; i++)
            {
                ...
                for (j = 0; j < fileCount; j++)
                {
                    String file = Path.Combine(subdir, $"testfile{j:D3}.a");
                    TestUtilities.CreateAndFillFile(file, _rnd.Next(100) + 500);
                    entries++;
                }
            }
        ...
        public void Create_SaveCancellation()
        {
            ...
            Directory.CreateDirectory(dirToZip);
            int subdirCount = 0;
            int entriesAdded = TestUtilities.GenerateFilesOneLevelDeep("Create_SaveCancellation", dirToZip, null, out subdirCount);
        ...
        public void Create_AddCancellation_wi13371()
        {
            ...
            Directory.CreateDirectory(dirToZip);
            int subdirCount = 0;
            int entriesAdded = TestUtilities.GenerateFilesOneLevelDeep("Create_AddCancellation", dirToZip, null, out subdirCount);
        ...
        public void ExtractAll_Cancellation()
        {
            ...
            Directory.CreateDirectory(dirToZip);
            int subdirCount = 0;
            int entriesAdded = TestUtilities.GenerateFilesOneLevelDeep("ExtractAll_Cancellation", dirToZip, null, out subdirCount);
        ...
        public void ExtractAll_WithPassword()
        {
            ...
            int entriesAdded = TestUtilities.GenerateFilesOneLevelDeep("ExtractAll_WithPassword", dirToZip, null, out subdirCount);
            string password = TestUtilities.GenerateRandomPassword();
            using (ZipFile zip1 = new ZipFile())
            {
        ...
        public void Extract_ImplicitPassword()
        {
            ...
            {
                string zipFileToCreate = Path.Combine(TopLevelDir, $"Extract_ImplicitPassword-{k}.zip");
                Directory.SetCurrentDirectory(TopLevelDir);
                string dirToZip = Path.GetFileNameWithoutExtension(Path.GetRandomFileName());
                var files = TestUtilities.GenerateFilesFlat(dirToZip);
        ...
        public void Extract_MultiThreaded_wi6637()
        {
            ...
            for (int k = 0; k < 1; k++)
            {
                ...
                {
                    zipFileToCreate[m] = Path.Combine(TopLevelDir, $"Extract_MultiThreaded-{k}-{m}.zip");
        ...
        public void Extended_CheckZip1()
        {
            ...
            for (int i = 0; i < crypto.Length; i++)
            {
                for (int j = 0; j < z64.Length; j++)
                {
                    ...
                    {
                        string zipFile = $"Extended-CheckZip1-{i}.{j}.{k}.zip";
        ...
        public void Extended_CheckZip2()
        {
            ...
            for (int i = 0; i < crypto.Length; i++)
            {
                for (int j = 0; j < z64.Length; j++)
                {
                    string zipFile = $"Extended-CheckZip2-{i}.{j}.zip";
                    string password = Path.GetRandomFileName();
        ...
        public void _Internal_DuplicateNames_DifferentFolders_wi8982(bool flat)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                string zipFileToCreate = Path.Combine(TopLevelDir, $"Create_DuplicateNames_DifferentFolders.{i}.zip");
        ...
        [ExpectedExceptionFact(typeof(System.IO.IOException))]
        ...
        public void Create_ZipErrorAction_Skip()
        {
            ...
            for (int m = 1; m < 4; m++)
            {
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    string zipFileToCreate = Path.Combine(TopLevelDir, $"Create_ZipErrorAction_Skip-{m}-{k}.zip");
        ...
        public void ParallelDeflateStream_Create_CompareSpeeds()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    string zipFileToCreate = Path.Combine(TopLevelDir, $"ParallelDeflateStream_Create.{i}.{j}.zip");
        ...
        [ExpectedExceptionFact(typeof(System.ArgumentOutOfRangeException))]
        ...
        {
            string zipFileToCreate = Path.Combine(TopLevelDir,
                $"AddDirectory_ReparsePoint-{flavor}.zip");
            string dirToZip = Path.GetFileNameWithoutExtension(Path.GetRandomFileName());
            var files = TestUtilities.GenerateFilesFlat(dirToZip);
